...
static void* filter_worker(FilterWorkerArgs* args) {
    ...
    for(int i = 0; i < args->size; ++i) {
        if(i > 1) {
            args->output[i] = x0 / 3 * args->input[i];
            args->output[i] += x1 / 3 * args->input[i - 1] - y1 / 3 * args->input[i - 1];
            args->output[i] += x2 / 3 * args->input[i - 2] - y2 / 3 * args->input[i - 2];
        }
        else if(i > 0) {
            args->output[i] = x0 / 2 * args->input[i];
            args->output[i] += x1 / 2 * args->input[i - 1] - y1 / 2 * args->input[i - 1];
        }
        else {
